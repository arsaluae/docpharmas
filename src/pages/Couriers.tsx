        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
          {stats.filter(s => s.count > 0 || s.key !== "__unassigned").map(s => (
            <button key={s.key} onClick={() => setSelectedProvider(s.key)}
              className={`group relative text-left p-4 rounded-2xl border bg-card/60 backdrop-blur-sm transition-all hover:shadow-lg hover:scale-[1.02] ${selectedProvider === s.key ? "border-primary/60 ring-2 ring-primary/30 shadow-lg" : "border-border/50"}`}>
              <div className="absolute inset-0 bg-gradient-to-br from-primary/5 via-transparent to-blue-500/5 opacity-0 group-hover:opacity-100 transition-opacity rounded-2xl" />
