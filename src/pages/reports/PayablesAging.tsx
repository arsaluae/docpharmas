...
interface AgingRow {
  bill_number: string; supplier: string; total: number;
...
export default function PayablesAging() {
  ...
  const load = async () => {
    ...
    const data: AgingRow[] = (inv.data || []).map(i => {
      ...
      return {
        bill_number: i.bill_number, supplier: nameMap[i.supplier_id || ""] || "—",
        total: Number(i.total), due_date: i.due_date || "—",
      ...
      <div className="min-h-screen flex w-full bg-background">
        ...
        <main className="flex-1 overflow-auto">
          <header className="sticky top-0 z-10 bg-background/80 backdrop-blur-sm border-b border-border px-6 py-4 flex items-center gap-4">
            <SidebarTrigger />
            <h1 className="text-xl font-bold text-foreground font-heading">Payables Aging</h1>
          </header>
            ...
            <Card className="glass-card">
              <CardContent className="p-0">
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead>Bill #</TableHead><TableHead>Supplier</TableHead><TableHead>Due Date</TableHead>
                      <TableHead>Days</TableHead><TableHead>Bucket</TableHead><TableHead className="text-right">Amount</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {rows.length === 0 ? (
