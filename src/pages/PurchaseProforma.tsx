...
import { Plus, Search, FileText, Trash2, Download, CheckCircle, Pencil, PackageCheck, MessageCircle, Wallet, Eye, Loader2, FileEdit, ShoppingCart, BadgeCheck, PackageOpen, RotateCcw, Truck, Send, MoreHorizontal, Banknote, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function PurchaseProforma() {
 ...
 supabase.from("purchase_proformas").select("*, suppliers(supplier_code, name, company, phone, address, city, area)").order("created_at", { ascending: false }),
 ...
 {(order.status === "draft" || ((order.status === "ordered" || order.status === "confirmed") && !order.grn_number && order.status !== "paid")) && <DropdownMenuItem onClick={() => openEditSheet(order)}><Pencil className="h-3.5 w-3.5 mr-2" /> Edit</DropdownMenuItem>}
