...
export default function StockAudit() {
 ...
 <Card className="glass-card"><CardContent className="p-4 flex items-center gap-3">
 <div className="h-10 w-10 rounded-xl bg-success/15 flex items-center justify-center"><CheckCircle2 className="h-5 w-5 text-success" /></div>
 <div><p className="text-xs text-muted-foreground">Reconciled</p><p className="text-lg font-bold font-mono text-success">{exactMatch}</p></div>
 </CardContent></Card>
 ...
 <TableCell className={`text-right font-mono font-semibold ${Math.abs(r.variance) >= 0.001 ? "text-destructive" : "text-success"}`}>
 {r.variance > 0 ? "+" : ""}{r.variance.toLocaleString()}
 </TableCell>
 <TableCell>
 {Math.abs(r.variance) < 0.001
 ? <Badge className="bg-success/15 text-success border-border">OK</Badge>
