...
const typePillCls = (type: EntryType) => {
  switch (type) {
    case "Sales Invoice":   return "bg-primary/10 text-primary border-primary/20";
    case "Payment Received":return "bg-success/10 text-success border-success/20";
    case "Sales Return":    return "bg-warning/10 text-warning border-warning/20";
    case "Credit Note":     return "bg-warning/10 text-warning border-warning/20";
    case "Warranty Invoice":return "bg-accent text-accent-foreground border-border";
...
export default function CustomerLedger() {
  ...
  const totalCredits = allEntries.filter(e => e.type === "Credit Note").reduce((s, e) => s + e.credit, 0);
  const outstanding = allEntries.length > 0 ? allEntries[allEntries.length - 1].balance : 0;
  const grossMax = Math.max(totalSales, 1);
  ...
  const aging = useMemo(() => {
    ...
    if (sum > 0) {
      ...
      Object.keys(buckets).forEach(k => { buckets[k] = Math.round(buckets[k] * ratio); });
    }
    return buckets;
  ...
  const handleRowClick = (e: LedgerEntry) => {
    ...
    switch (e.type) {
      case "Sales Invoice":    navigate(`/proforma?invoice=${e.source_id}`); break;
      case "Payment Received": navigate(`/payments?tab=received&highlight=${e.source_id}`); break;
      case "Sales Return":     navigate(`/sales-returns?highlight=${e.source_id}`); break;
      case "Warranty Invoice": navigate(`/warranty-invoices?highlight=${e.source_id}`); break;
      case "Credit Note":      navigate(`/credit-notes?highlight=${e.source_id}`); break;
    }
  };
  ...
  const shareViaWhatsApp = () => {
    if (!customer?.phone) { toast.error("No phone number on record"); return; }
    const msg = `📊 *Ledger — ${customer.name}*\n\nSales: PKR ${totalSales.toLocaleString()}\nReceived: PKR ${totalReceived.toLocaleString()}\nReturns: PKR ${totalReturns.toLocaleString()}\n*Outstanding: PKR ${outstanding.toLocaleString()}*\n\nAs of ${fmtDate(new Date().toISOString())}`;
    window.open(`https://api.whatsapp.com/send?phone=${customer.phone.replace(/[^0-9]/g, "")}&text=${encodeURIComponent(msg)}`, "_blank");
  };

  const kpis = [
    { label: "Total Sales", value: totalSales, color: "text-foreground", bar: "bg-primary/50" },
    { label: "Received",     value: totalReceived, color: "text-success",  bar: "bg-success/50" },
    { label: "Returns",      value: totalReturns,  color: "text-warning",  bar: "bg-warning/50" },
    { label: "Credit Notes", value: totalCredits,  color: "text-muted-foreground", bar: "bg-muted-foreground/40" },
  ];
  ...
  return (
    <AppLayout title={`${customer?.name || "Customer"} — Ledger`} subtitle={customer?.company || customer?.city || ""} headerActions={headerActions}>
        ...
        <div className="p-6 md:p-8 border-b border-border flex flex-col md:flex-row md:items-end justify-between gap-6">
          ...
          <div className="flex flex-col md:items-end">
            ...
            <div className="flex gap-2 mt-4">
              <Button size="sm" variant="outline" onClick={exportCSV} className="h-8 text-xs gap-2">
                <Download className="h-3.5 w-3.5" /> Export CSV
              </Button>
              <Button size="sm" variant="outline" onClick={shareViaWhatsApp}
                className="h-8 text-xs gap-2 text-success border-success/30 hover:bg-success/10">
            ...
            {kpis.map(k => {
              ...
              return (
                <div key={k.label} className="p-5 md:p-6 border-r border-border last:border-r-0">
                  ...
                  <div className={`text-xl font-medium tabular-nums tracking-tight ${k.color}`}>
          ...
          <div className="p-5 md:p-6 bg-primary/[0.03] border-t lg:border-t-0 lg:border-l border-border">
            ...
            <div className="flex h-8 gap-0.5 rounded overflow-hidden">
              {Object.entries(aging).map(([bucket, amt], i) => {
                ...
                return amt > 0
                  ? <div key={bucket} className={tone} style={{ width: `${pct}%` }} title={`${bucket}: ${amt.toLocaleString()}`} />
                  : <div key={bucket} className="flex-1 bg-border/40" />;
              })}
            </div>
              ...
              {DATE_FILTERS.map(d => (
                <button key={d.key} onClick={() => setDateRange(d.key)}
                  className={`px-3 py-1 text-xs font-medium rounded transition-colors ${
                    dateRange === d.key ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"
                  }`}>
                  {d.label}
                </button>
              ))}
              ...
              {TYPE_FILTERS.map(t => (
                <button key={t.key} onClick={() => setTypeFilter(t.key)}
                  className={`text-xs pb-0.5 transition-colors ${
                    typeFilter === t.key ? "text-primary border-b border-primary font-medium" : "text-muted-foreground hover:text-foreground"
                  }`}>
                  {t.label}
                </button>
              ))}
                ...
                return (
                  <tr key={i}
                    ...
                    <td className="py-3.5 px-4">
                      <span className={`px-2 py-0.5 rounded-sm text-[10px] font-bold tracking-tight uppercase border ${typePillCls(e.type)}`}>
                        {e.type}
                      </span>
                    </td>
                    ...
                    <td className="py-3.5 px-4 text-right font-mono text-[13px] tabular-nums">
                      {e.debit > 0 ? <span className="text-foreground">{e.debit.toLocaleString()}</span> : <span className="text-muted-foreground/60">—</span>}
                    </td>
                    <td className="py-3.5 px-4 text-right font-mono text-[13px] tabular-nums">
                      {e.credit > 0 ? <span className="text-success">{e.credit.toLocaleString()}</span> : <span className="text-muted-foreground/60">—</span>}
                    </td>
