...
export default function CustomerLedger() {
  ...
  const loadLedger = async (customerId: string) => {
    ...
    if (cust) {
      raw.push({ date: cust.created_at.split("T")[0], type: "Opening Balance", reference: "—", debit: Number(cust.opening_balance), credit: 0 });
    }
        ...
        <main className="flex-1 overflow-auto">
          <header className="sticky top-0 z-10 bg-background/80 backdrop-blur-sm border-b border-border px-6 py-4 flex items-center gap-4">
            ...
            <div className="flex-1">
              <h1 className="text-xl font-bold text-foreground font-heading">{customer?.name || "Customer"} — Ledger</h1>
              <p className="text-sm text-muted-foreground">{customer?.company || ""} {customer?.city ? `• ${customer.city}` : ""}</p>
            </div>
          </header>
