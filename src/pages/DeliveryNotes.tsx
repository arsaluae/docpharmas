...
export default function DeliveryNotes() {
 ...
 if (l.includes("ncc")) return "bg-primary/15 text-primary border-border";
 if (l.includes("adda")) return "bg-warning/15 text-warning border-border";
 if (l.includes("self")) return "bg-success/15 text-success border-border";
 ...
 <p className="text-2xl font-bold font-heading text-warning mt-1">{issuedCount}</p>
 </div>
 <div className="p-4 rounded-xl border border-border bg-card">
 <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-widest">Delivered</p>
 <p className="text-2xl font-bold font-heading text-success mt-1">{deliveredCount}</p>
 </div>
 ...
 className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all capitalize ${courierFilter === c ? "bg-primary text-primary shadow-sm border border-primary/20" : "text-muted-foreground hover:text-foreground hover:bg-muted/60"}`}>
 ...
 <TableCell onClick={() => openDetail(dn)}>
 <Badge variant="outline" className={`text-[10px] font-semibold ${dn.status === "delivered" ? "bg-success/15 text-success border-border" : "bg-warning/15 text-warning border-border"}`}>
 {dn.status}
 </Badge>
 </TableCell>
 <TableCell>
 ...
 <Button variant="ghost" size="icon" className="h-7 w-7" onClick={async () => {
  ...
  if (dn.customer_id) {
  const { data } = await supabase.from("customers").select("phone, city, address, customer_code").eq("id", dn.customer_id).single();
  ...
  document_date: dn.date,
  document_status: dn.status,
 ...
 }} className="text-xs h-7 gap-1 bg-success/10 hover:bg-success/10 text-white">
 ...
 {selected.size > 0 && (
 <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-foreground text-background px-6 py-3 rounded-md flex items-center gap-3 z-50">
